...
2. 对每个**带 prefix 可路由**的提供商，发一个**真实请求**（默认提示词：「写一个解压zip文件的python脚本，只要核心代码」），看它现在到底能不能用。
3. 把「实时探测结果」+「历史健康度」+「跨轮状态」结合，算出目标优先级档位。
4. 仅当优先级（或 openai-compatibility 的 `disabled`）确实变化时，通过 CPA 管理 API 以 **整表 PUT** 的方式原子回写。
...
`prober.classify()` 把一次探测映射为四种语义，是「区分临时失效与永久失效」的关键：
...
- 仅当值确实变化才 PUT。
- 探测失败永不单独判死，必须 DB 也确认，且需连续 `dead_streak` 轮。
- 永不自动**重新启用**用户手动禁用的条目——除非有正向证据（探测 OK 或 DB 明确成功）。
...
- `CPA_CLIENT_API_KEY`：`config.yaml` 里 `api-keys:` 下的任一客户端 key（探测时用）
...
> **首次清理建议连跑两轮 `--once`**：因为判死需要 `dead_streak`（默认 2）轮连续确认，第一轮把失效中转降到 `poor`，第二轮才会真正置 `dead`/`disabled`。这是有意的防抖设计。
